...
export default function PremiumPage() {
  ...
  const handleSimulateUpgrade = async () => {
    ...
    try {
        ...
        toast({
            title: 'Félicitations ! 🎉',
            description: 'Vous êtes maintenant un membre Premium. Toutes les fonctionnalités ont été débloquées.',
        });
        ...
        <Card className="glassmorphism shadow-xl max-w-2xl w-full mt-6 border-2 border-yellow-400/50">
            ...
            <CardContent className="p-8">
                ...
                {userData?.subscription_type === 'premium' ? (
                  ...
                  >
                      {isUpgrading ? <><Loader className="w-5 h-5 mr-3 animate-spin"/> Mise à niveau...</> : <><Rocket className="w-5 h-5 mr-3" />Mettre à niveau (Simulation)</>}
                  </Button>
                )}
